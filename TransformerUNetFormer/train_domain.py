"""
Domain-specific training wrapper for TransformerUNetFormer (ResNet18).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Domain-specific training for TransformerUNetFormer.")
    parser.add_argument("--domain", choices=["urban", "rural"], required=True)
    parser.add_argument("--variant", default="resnet18")
    ...
    if args.variant != "resnet18":
        raise ValueError("TransformerUNetFormer uses ResNet18 only.")

    paths = _domain_paths(args.domain)
    output_dir = _build_output_dir(args.out_root, "transformerunetformer", args.variant, args.domain, args.crop, args.tag)
