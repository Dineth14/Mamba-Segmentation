"""
train.py — Iteration-based training loop for TransformerSwinTiny.
...
def setup_logging(output_dir: str) -> logging.Logger:
    ...
    # Create logger
    logger = logging.getLogger("TransformerSwinTiny")
...
def validate(
    ...
    for batch_idx, batch in enumerate(val_pbar):
        ...
        if expected_size is not None and rgb.shape[-2:] != (expected_size, expected_size):
            rgb = F.interpolate(rgb, size=(expected_size, expected_size), mode="bilinear", align_corners=False)
            mask = F.interpolate(mask.unsqueeze(1).float(), size=(expected_size, expected_size), mode="nearest")
...
def train(config: Config, resume_path: str = None):
    ...
    logger.info(f"Swin variant: {config.SWIN_VARIANT}")
    logger.info(f"Swin embed dim: {config.SWIN_EMBED_DIM}")
    logger.info(f"Swin depths: {config.SWIN_DEPTHS}")
    logger.info(f"Swin heads: {config.SWIN_NUM_HEADS}")
    logger.info(f"Swin drop path: {config.SWIN_DROP_PATH}")
    ...
    for iteration in train_pbar:
        ...
        if (iteration + 1) % config.VAL_INTERVAL == 0:
            ...
            metrics = validate(
                ...
                expected_size=config.CROP_SIZE,
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TransformerSwinTiny segmentation.")
    parser.add_argument(
        "--config",
        default="loveda",
