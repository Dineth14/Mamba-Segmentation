"""
Domain-specific training wrapper for TransformerSwinTiny.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Domain-specific training for TransformerSwinTiny.")
    parser.add_argument("--domain", choices=["urban", "rural"], required=True)
    parser.add_argument("--variant", default="swin_tiny")
    ...
    if args.variant not in ("swin_tiny", "tiny"):
        raise ValueError("TransformerSwinTiny uses Swin-Tiny only.")

    paths = _domain_paths(args.domain)
    output_dir = _build_output_dir(args.out_root, "transformer_swintiny", args.variant, args.domain, args.crop, args.tag)
