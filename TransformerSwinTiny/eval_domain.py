...
def _clear_thop_hooks(model: torch.nn.Module) -> None:
    """Remove any THOP hooks/attributes to avoid interfering with evaluation."""
...
def main() -> None:
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if rgb.shape[-2:] != (cfg.CROP_SIZE, cfg.CROP_SIZE):
                rgb = F.interpolate(
                    rgb,
