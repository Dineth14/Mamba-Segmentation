#!/usr/bin/env python3
"""
Benchmark inference FPS and GPU memory for All->All segmentation experiments.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark FPS and VRAM for All->All segmentation experiments."
    )
    ...
    parser.add_argument("--pattern", default=None, help="Optional include regex")
    parser.add_argument("--exclude", default=None, help="Optional exclude regex")
...
def scan_experiments(
    ...
    for entry in root_path.iterdir():
        ...
        if include_re and not include_re.search(entry.name):
            continue
        if exclude_re and exclude_re.search(entry.name):
            continue
...
def benchmark_model(
    ...
    return {
        ...
        "peak_mem_reserved_gb": peak_reserved_gb,
    }
...
def print_summary(rows: List[Dict[str, Any]]) -> None:
    ...
    def _fmt(row: Dict[str, Any]) -> str:
        return (
            ...
            f"{row['variant']:<8} "
            f"fps={row['fps']:.2f} "
            f"ms={row['ms_per_img']:.2f} "
            f"mem={row['peak_mem_alloc_gb']:.2f}GB"
        )

    by_fps = sorted(rows, key=lambda r: r["fps"], reverse=True)
    by_mem = sorted(rows, key=lambda r: r["peak_mem_alloc_gb"])

    print("\n=== Sorted by FPS (desc) ===")
    for row in by_fps:
        print(_fmt(row))

    print("\n=== Sorted by Peak VRAM (asc) ===")
    for row in by_mem:
        print(_fmt(row))
...
def main() -> int:
    ...
    include_re = re.compile(args.pattern) if args.pattern else None
    exclude_re = re.compile(args.exclude) if args.exclude else None

    out_path = Path(args.out) if args.out else Path(args.root) / "fps_mem_allall.csv"
    ...
    exp_dirs = scan_experiments(args.root, include_re, exclude_re)
    if not exp_dirs:
