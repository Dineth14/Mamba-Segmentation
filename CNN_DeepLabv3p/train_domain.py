"""
Domain-specific training wrapper for CNN_DeepLabv3p (ResNet50).
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Domain-specific training for CNN_DeepLabv3p.")
    parser.add_argument("--domain", choices=["urban", "rural"], required=True)
    parser.add_argument("--variant", default="resnet50")
    ...
    if args.variant != "resnet50":
        raise ValueError("CNN_DeepLabv3p uses ResNet50 only.")

    paths = _domain_paths(args.domain)
    output_dir = _build_output_dir(args.out_root, "cnn_deeplabv3p", args.variant, args.domain, args.crop, args.tag)
