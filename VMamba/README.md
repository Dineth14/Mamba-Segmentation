...
```
RGB Image (3ch) ──► RGBEncoder (VMamba: tiny/small/base) ──► LightUNetDecoder ──► Predictions
```
...
- **RGB-only Architecture**: VMamba for RGB features + lightweight U-Net decoder
- **VMamba Variants**: Select between tiny, small, and base via config
...
VMamba pretrained weights should be placed under the weights directory and selected via config:
```
